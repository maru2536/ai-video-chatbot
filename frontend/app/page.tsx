...
export default function Home() {
  ...
  }, [message, messages, saveConversation, speechSynthesisSupported, speakText, useRAG])
  ...
  const handleFileUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError({
          type: 'api',
          message: errorData.error || 'アップロードに失敗しました',
          retryable: false
        })
      }
    } catch (error) {
          ...
          <div>
            ...
            {useRAG && uploadedFiles.length > 0 && (
              <p className="text-sm text-gray-600 mt-1">
                📚 ドキュメント参照モード ({uploadedFiles.length}ファイル)
              </p>
            )}
          </div>
          ...
          <div className="flex gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {uploadedFiles.length > 0 && (
                <span className="text-xs text-green-600">({uploadedFiles.length}ファイル)</span>
              )}
            </div>
            
            {/* ファイルアップロードボタン */}
            <label className="btn-simple px-4 py-2 text-sm font-medium cursor-pointer">
